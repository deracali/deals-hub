...
const DealSubmission = () => {
  ...
  const handleSubmit = async (submissionData: DealFormData) => {
    ...
    try {
      // Re-fetch latest user data via service to ensure up-to-date type/counts
      const latestUser = userId ? await fetchUserProfile(userId) : null;

      // Use service to submit deal (service will detect platform)
      const { data: dealData, platform } = await submitDeal(
        submissionData,
        userId,
      );

      // Update counts depending on platform & user type
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="space-y-6">
              ...
              <SubmissionActions
                ...
                agreementChecked={agreementChecked} // ← new prop
                setAgreementChecked={setAgreementChecked} // ← new prop
              />
            </div>
          </div>
