...
const [cartItems, setCartItems] = useState<any[]>([]);
  ...
  return (
    ...
    <div className="container mb-4 mx-auto px-4 py-3 pt-6 flex items-center justify-between">
      ...
      <div className="flex items-center space-x-3">
        ...
        <div className="hidden md:flex items-center space-x-4">
          ...
          {/* --- ALWAYS VISIBLE: Cart Icon & Modal Trigger --- */}
          <div className="relative">
            ...
            >
              <div className="relative">
                <Image src="/Handbag.png" alt="Cart" width={18} height={18} />
                {cartBadgeCount > 0 && (
                  <span className="absolute -top-1 -right-1 min-w-[14px] h-[14px] text-[9px] font-bold text-white bg-[#FE811D] rounded-full flex items-center justify-center">
                    {cartBadgeCount}
                  </span>
            ...
            <div className="relative">
              ...
              {isUserMenuOpen && (
                <div className="absolute right-0 mt-4 w-48 bg-white rounded-xl shadow-xl py-2 px-1 border border-gray-50 animate-in fade-in zoom-in duration-150">
                  <Link
                    href={user?.role?.toLowerCase() === "admin" ? "/admin" : user?.type === "vendor" ? "/vendor" : "/profile"}
                    onClick={() => setIsUserMenuOpen(false)}
                    className="flex items-center gap-3 px-4 py-2.5 text-sm text-gray-500 hover:bg-gray-50 rounded-lg"
                  >
                    <User size={16} />
                    {user?.role?.toLowerCase() === "admin" ? "Admin Dashboard" : user?.type === "vendor" ? "Vendor Dashboard" : "My account"}
                  </Link>
                  <Link href="/history" className="flex items-center gap-3 px-4 py-2.5 text-sm text-gray-500 hover:bg-gray-50 rounded-lg">
                    <Package size={16} /> Orders
                  </Link>
                  <Link href="/wishlist" className="flex items-center gap-3 px-4 py-2.5 text-sm text-gray-500 hover:bg-gray-50 rounded-lg">
    ...
    <div className="md:hidden bg-white px-4 pt-3 pb-4 space-y-4">
      ...
      {/* Mobile Quick Actions (Red, Green, Blue, Yellow circles) */}
      <div className="flex items-center justify-between">
        {[
          { label: "Deals", img: "/Group 27.png", bg: "bg-red-50", path: "/deals" },
          { label: "Submit Deals", img: "/Group 26.png", bg: "bg-green-50", path: "/deals/post" },
          { label: "Vendors", img: "/Group 30.png", bg: "bg-blue-50", path: "/vendors" },
          { label: "Community", img: "/Group 31.png", bg: "bg-yellow-50", path: "/forum" },
        ].map((item) => (
          <Link key={item.label} href={item.path} className="flex flex-col items-center gap-1">
        ...
        <div className="px-6 py-8 border-b bg-gray-50">
          {!isAuthenticated ? (
            <div className="flex flex-col gap-4">
              <h3 className="text-lg font-bold">Welcome to Slyce</h3>
              <div className="flex items-center gap-3">
                <Link href="/login" onClick={() => setIsMenuOpen(false)} className="flex-1 py-3 text-center border-2 border-[#0d9cff] text-[#0d9cff] font-bold rounded-xl text-sm">Log in</Link>
                <Link href="/join" onClick={() => setIsMenuOpen(false)} className="flex-1 py-3 text-center bg-[#0d9cff] text-white font-bold rounded-xl text-sm shadow-md">Register +</Link>
              </div>
            </div>
          ) : (
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-4">
                <div className="w-14 h-14 relative rounded-full overflow-hidden border-2 border-[#0d9cff]">
                  {(user as any)?.photo ? <Image src={(user as any).photo} alt="avatar" fill className="object-cover" /> : <div className="w-full h-full bg-[#0d9cff] text-white flex items-center justify-center font-bold text-xl">{avatarInitials(user as UserLocal)}</div>}
                </div>
                <div>
                  <h3 className="font-bold text-gray-900 text-lg">Hi, {(user as any)?.firstName || getDisplayName(user as UserLocal)}</h3>
                  <p className="text-xs text-gray-500">Lagos, Nigeria</p>
                </div>
              </div>
              <button onClick={handleLogout} className="p-3 bg-red-50 text-red-500 rounded-full"><LogOut size={20} /></button>
            </div>
          )}

          {/* --- ALWAYS VISIBLE ON MOBILE MENU --- */}
          <div className="flex items-center gap-4 mt-6">
            <Link href={isAuthenticated ? "/wishlist" : "/login"} onClick={() => setIsMenuOpen(false)} className="w-12 h-12 rounded-full bg-[#5EB3FD] flex items-center justify-center relative shadow-sm">
              <Image src="/ChatCircleText.png" alt="Saved" width={22} height={22} />
              {savedDealsCount > 0 && <span className="absolute -top-1 -right-1 min-w-[18px] h-[18px] text-[10px] font-bold text-white bg-[#FE811D] rounded-full flex items-center justify-center border-2 border-white">{savedDealsCount}</span>}
            </Link>
            <Link href="/deals/cart" onClick={() => setIsMenuOpen(false)} className="w-12 h-12 rounded-full bg-[#5EB3FD] flex items-center justify-center relative shadow-sm">
              <Image src="/Handbag.png" alt="Cart" width={22} height={22} />
              {cartBadgeCount > 0 && <span className="absolute -top-1 -right-1 min-w-[18px] h-[18px] text-[10px] font-bold text-white bg-[#FE811D] rounded-full flex items-center justify-center border-2 border-white">{cartBadgeCount}</span>}
            </Link>
    ...
    {/* Bottom Nav (Coupons, Become a Vendor, etc) */}
    <div className="w-full bg-white border-t border-gray-100 hidden md:block">
      <div className="container mx-auto px-4 py-2 flex items-center justify-between">
        <div className="flex items-center gap-6 text-gray-600 text-[13px] font-medium">
          <Link href="/coupons" className="hover:text-[#0d9cff]">Coupons</Link>
          <Link href="/vendors/become" className="flex items-center gap-2 hover:text-[#0d9cff]">
            <Image src="/Group 30.png" alt="Vendor" width={16} height={16} />
            <span>Become a Vendor</span>
          </Link>
          <Link href="/special-offers" className="flex items-center gap-2 px-3 py-1.5 bg-[#fff8f0] text-[#ff7a00] rounded-lg">
